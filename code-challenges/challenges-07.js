...
const objLat = (obj) => {
    ...
    const format = `my name is ${first_l(obj.firstName)} ${first_l(obj.lastName)} I am ${obj.age} YO, and I love ${obj.hobby}.`;
...
const cvFormatter = (arr) => {
    ...
    for (const i of arr) {
        if (i.yearsOfExperience > 1) {
            const fullName = (i.firstName && i.lastName) ? `${i.firstName} ${i.lastName}` : i.firstName || i.lastName;
            arr2.push({
                fullName: fullName,
                tech: i.tech
            });
